...
export default function StarRating({maxRating=5,color="#fcc419",size="48",className="",messages=[],defaultRating=0,onsetRating }){
    ...
    return <div style={containerStyle} className={className}>
        <div style={starcontainerStyle} >
                {Array.from({length:maxRating},(_,i)=><Star color={color} size={size} key={i} onRate={()=>handleRating(i+1)} full={ temprating ? temprating>= i+1 :rating >= i+1} onHoverIn={()=>settempRating(i+1)} onHoverOut={()=>settempRating(0)}/>)}    
        </div>
